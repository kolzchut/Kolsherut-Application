def build_rank_contributions(ranked_documents: list[dict], rank_constant: int) -> dict:
    return {
        document['card_id']: 1 / (rank_constant + rank)
...
def sum_reciprocal_ranks(ranked_document_lists: list[list[dict]], rank_constant: int) -> dict:
    ...
    for ranked_documents in ranked_document_lists:
        ...
        for card_id, contribution in contributions.items():
            fused_scores[card_id] = fused_scores.get(card_id, 0.0) + contribution
    return fused_scores


def index_documents_by_card_id(ranked_document_lists: list[list[dict]]) -> dict:
    return {
        document['card_id']: document
        for ranked_documents in ranked_document_lists
...
def fuse_rankings_by_reciprocal_rank(
    ...
    documents_by_card_id = index_documents_by_card_id(ranked_document_lists)
    fused_documents = [
        {**documents_by_card_id[card_id], 'score': fused_score}
        for card_id, fused_score in fused_scores.items()
    ]
