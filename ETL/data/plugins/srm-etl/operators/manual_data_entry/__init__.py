...
from operators.manual_data_entry.mde_utils import load_manual_data
from operators.manual_data_entry.external import main as load_external_data
...
def mde_prepare():
    ...
    source_flow = DF.Flow(
        load_from_airtable(settings.AIRTABLE_DATAENTRY_BASE, settings.AIRTABLE_SERVICE_TABLE, settings.AIRTABLE_VIEW,
        ...
        stats.filter_with_stat('Manual Data Entry: No Org ID or Org Name',
                               lambda r: (r.get('Org Id') or r.get('Org Name')), report=unknown_entity_ids),
        DF.add_field('_row_id', 'string', lambda r: r[AIRTABLE_ID_FIELD]),
    )

    data_sources = DF.Flow(
        load_from_airtable(settings.AIRTABLE_DATAENTRY_BASE, 'DataReferences', settings.AIRTABLE_VIEW,
                           settings.AIRTABLE_API_KEY),
    ).results()[0][0]
