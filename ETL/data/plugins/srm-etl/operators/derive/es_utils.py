...
def dump_to_es_and_delete(**kwargs):
    ...
    kwargs.setdefault('index_settings', {
        "analysis": {
            "analyzer": {
                "hebrew": {
                    "tokenizer": "icu_tokenizer",  # Requires the analysis-icu plugin
                    "filter": [
                        "icu_folding",  # Handles Hebrew letter normalization better than standard
                        "icu_normalizer"
                    ]
                }
            }
    ...
    def deleter():
        ...
        for index in indexes:
            response = engine.delete_by_query(index, body=dict(
                query=dict(bool=dict(must_not=dict(term=dict(revision=unique_id))))), conflicts='proceed')
