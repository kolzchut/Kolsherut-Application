"""Stage 5 - publish the cards and autocomplete indexes (legacy to_es.py).

Only srm__cards and srm__autocomplete are published - the four indexes with no
consumer (places, responses, situations, orgs) were dropped (deliberate
behavior change #5). Search-only fields are added on document copies so the
in-memory cards stay Airtable-shaped. The frozen mappings arrive preloaded
from the pipeline-start preflight.
"""
...
from .mappings.index_mappings import AUTOCOMPLETE_INDEX_NAME, CARDS_INDEX_NAME
