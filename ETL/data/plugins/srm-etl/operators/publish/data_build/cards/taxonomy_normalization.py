"""Taxonomy id cleanup (legacy normalize_taxonomy_ids, map_taxonomy, fix_gender_and_language_situations)."""
import re
...
CONCATENATED_IDS_PATTERN = re.compile(r'human_situations:[A-Za-z0-9_:-]+|human_situation:[A-Za-z0-9_:-]+')
...
def split_raw_taxonomy_value(raw):
    ...
    for part in comma_parts:
        ...
        if part.count(PLURAL_ROOT_PREFIX) + part.count(SINGULAR_ROOT_PREFIX) > 1:
            smashed_tokens = CONCATENATED_IDS_PATTERN.findall(part)
            if smashed_tokens:
...
def fix_gender_and_language_situations(ids):
    if not ids:
        return ids
    fixed = list(ids)
