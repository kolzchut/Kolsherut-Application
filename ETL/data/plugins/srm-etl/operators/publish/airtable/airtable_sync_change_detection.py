"""Pure change detection for the Airtable sync (legacy airtable_updater semantics).

Fetched rows are merged with their current main-base twins; only new or changed
rows (legacy hash comparison) are kept for writing, and rows that vanished from
the fetch become INACTIVE.
"""
from .airtable_client import AIRTABLE_RECORD_ID_FIELD
