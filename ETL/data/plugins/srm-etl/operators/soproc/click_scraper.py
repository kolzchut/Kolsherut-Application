...
def filter_results():
    return DF.Flow(
        ...
        DF.filter_rows(
            lambda row: row.get('distribution_channel') is not None and row.get('distribution_channel')[0] == 1),
    )
    ...
    records = DF.Flow(
        ...
        DF.set_type('details',
                    transform=lambda _, row: ''.join('<p>{}</p>'.format(row[f].strip()) for f in [
                        'description', 'details', 'implementation_details', 'target_community_text',
        ...
        DF.add_field('links', 'array',
                     lambda r: list(filter(None, [v for k, v in r.items() if k.startswith('link_')]))),
        DF.select_fields(
            ['catalog_number', 'name', 'description', 'details', 'payment_required', 'payment_details', 'data_sources',
             'urls']),
        # DF.printer()
    ).results()[0][0]
