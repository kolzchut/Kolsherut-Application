...
from app.services.elasticsearch.fetch_card_service_ids import fetch_card_service_ids
...
from app.vars import REQUIRE_CARD_FOR_EMBEDDING, SERVICE_ID_FIELD_NAME
...
def keep_services_with_cards(services: Iterator[dict], card_service_ids: set) -> Iterator[dict]:
    return (service for service in services if service[SERVICE_ID_FIELD_NAME] in card_service_ids)


def select_services_to_index(limit: Optional[int] = None, resume: bool = False) -> Iterator[dict]:
    services = scan_all_services()
    if REQUIRE_CARD_FOR_EMBEDDING:
        services = keep_services_with_cards(services, fetch_card_service_ids())
    if resume:
