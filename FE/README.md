...
│   ├── sitemap/                # generated hpsitemap.xml (prebuild output)
...
└── src/
    ...
    ├── pages/                  # one folder per page: home, results, card, maintanence
    │   └── pages.ts            # page-key → component map (home, map, card, results, maintenance)
...
`App.tsx` selects the current page key from the store (`general` slice) and renders the matching component from `pages/pages.ts` (`home`, `map` → home with map modal, `card`, `results`, `maintenance`).
...
4. **`components/sitemap.cjs` — route discovery**: fetches `sitemap.xml` (local first, falls back to the remote target domain), extracts sub-sitemaps (`cards.xml`, `taxonomy.xml`, `mixedtaxonomy.xml`, `hpsitemap.xml`, `services.xml`, `organizations.xml`), and collects every `<loc>` whose domain is allowed into a route set (always including `/`). Note: card/taxonomy sub-sitemaps are generated by the **BE** and reached through the nginx proxy on the deployed site, which is why the remote fallback matters.
...
1. nginx classifies the client by User-Agent (`$is_bot` map): Googlebot, Bingbot, Yandex, DuckDuckBot, social preview bots (Facebook, Twitter/X, LinkedIn, WhatsApp, Telegram, Discord, Apple), plus `curl`/`wget`/`python-requests`. The special UA `KolSherutBot` is explicitly **not** a bot — that is the UA the SSR renderer itself uses, preventing infinite proxy loops.
...
**Generated at build time** (both run in `prebuild`):
- `scripts/generateMainSitemap.cjs` → `public/sitemap.xml` — the **sitemap index**, listing six sub-sitemaps with `<loc>` on the env's `currentURL`. `cards.xml` gets a fresh `<lastmod>`; the rest use `sitemapsDefaultLastModified`.
...
npm run build:<env>
  └─ build:<env>:base
  │    └─ prebuild   → sitemap:homepage + sitemap:main + synonyms:update
...
| `lint` | ESLint |
| `docker:build:<env>` | full build + `docker build` (image `kolsherut_fe:<version>-<env>`) |
...
| `sitemap:homepage` / `sitemap:main` / `synonyms:update` | the prebuild steps, runnable standalone |
