from evaluation import human_review_strings, relevance_statistics_vars, relevance_vars
...
def build_stratum_keys() -> list[StratumKey]:
    ...
    vocabularies - the sides Mission 5 aggregates over and the verdicts the judge may return - so a
    new verdict becomes a new stratum with no edit here.
    """
    return [(side, verdict)
...
def group_pairs_by_stratum(judged_pairs: list[JudgedPair]) -> dict[StratumKey, list[JudgedPair]]:
    ...
    for item, judgement in judged_pairs:
        groups[(judgement.side, judgement.verdict)].append((item, judgement))
    return groups
